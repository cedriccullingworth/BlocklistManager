...
internal class TextDataCollector : IDisposable
{
    ...
    public string ReadData( RemoteSite site, out string? fileExtension, string url, string logFilePath = "" )
    {
        string textData = "";
        fileExtension = string.Empty;
        return DownloadText( site, url, ref textData, logFilePath );
    }

    public Stream? ReadHtmlStreamFromUrl( RemoteSite site, string url, string logFilePath = "" )
    {
        ...
        catch ( Exception e )
        {
            ...
            else
            {
                if ( e.InnerException is not null )
                    Logger.Log( _appName, $"Stream download attempt from {site.Name} failed, {e.InnerException.Message}" );
                else
                    Logger.Log( _appName, $"Stream download attempt from {site.Name} failed, {e.Message}" );
            }
    ...
    private string DownloadText( RemoteSite site, string url, ref string textData, string logFilePath = "" )
    {
    ...
    public string ReadZipData( RemoteSite site, out string fileExtension, string url )
    {
        fileExtension = string.Empty;
        ...
        Stream? stream = this.ReadHtmlStreamFromUrl( site, url );
